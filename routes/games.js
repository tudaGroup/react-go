...
router.patch('/games/:id', auth, async (req, res) => {
  ...
  if (game.rated) {
    if (player1Won) {
      newRatingPlayer1 = Math.max(
        oldRatingPlayer1 +
          5 +
          15 * Math.min(oldRatingPlayer2 / (oldRatingPlayer1 + 1), 1),
        0
      );
      newRatingPlayer2 = Math.max(
        oldRatingPlayer2 -
          (5 + 15 * Math.min(oldRatingPlayer2 / (oldRatingPlayer1 + 1), 1)),
        0
      );
    } else {
      newRatingPlayer1 = Math.max(
        oldRatingPlayer1 -
          (5 + 15 * Math.min(oldRatingPlayer1 / (oldRatingPlayer2 + 1), 1)),
        0
      );
      newRatingPlayer2 = Math.max(
        oldRatingPlayer2 +
          5 +
          15 * Math.min(oldRatingPlayer1 / (oldRatingPlayer2 + 1), 1),
        0
      );
    }
